...
function populateSVGcanvas() {
    ...
    // Define a color scale that maps count values to a color gradient
    const color = d3.scaleSequential(d3.interpolateOranges) // Choose a gradient
        .domain([0, d3.max(ctx.data, d => +d.count)]); // Map to the range of counts
    ...
    node.append("circle")
        ...
        .attr("fill", d => color(+d.data.count)) // Use the color scale based on count
